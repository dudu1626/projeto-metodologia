...
# Constantes para o modelo LeNet
LENET_TARGET_ACCURACY = 0.75
# observado é 0.729
...
# Cria o diretório pai 'MobileNet' com incremento, se necessário
nome_modelo = 'MobileNet-FP32-limitado'
diretorio_pai = criar_diretorio_incrementado('resultadosMobileNet', nome_modelo)
...
class Block(nn.Module):
    '''Convolução separável em profundidade (Depthwise) + Convolução Ponto a Ponto (Pointwise)'''
    def __init__(self, in_planes, out_planes, stride=1):
        super(Block, self).__init__()
        # Convolução Depthwise
        self.conv1 = nn.Conv2d(in_planes, in_planes, kernel_size=3, stride=stride, padding=1, groups=in_planes, bias=False)
        self.bn1 = nn.BatchNorm2d(in_planes)
        # Convolução Pointwise
...
def treinar_e_validar(modelo, carregador_treino, carregador_validacao, criterio, otimizador, epocas, i, target_accuracy=None):
    ...
    for epoca in range(epocas):
        ...
        if target_accuracy is not None and acuracia_validacao >= target_accuracy:
            print(f"Parada antecipada: Acurácia de validação ({acuracia_validacao:.4f}) atingiu ou excedeu a acurácia alvo da LeNet ({target_accuracy:.4f}) na época {epoca + 1}")
            break
...
for i in range(numero_modelos):
    ...
    perda_treino, acuracia_treino, perda_validacao, acuracia_validacao, tempo_treino, consumo_energia = (
        treinar_e_validar(modelo, carregador_treino, carregador_validacao, criterio, otimizador, maximo_epocas, i, target_accuracy=LENET_TARGET_ACCURACY))
...
# @track_emissions(project_name="MobileNet_original", output_dir=diretorio_carbon)
