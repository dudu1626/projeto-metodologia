...
nome_modelo = 'AlexNet-FP32_limitado'
...
def treinar_e_validar(modelo, carregador_treino, carregador_validacao, criterio, otimizador, epocas, i, target_accuracy=None):
    ...
    for epoca in range(epocas):
        ...
        if target_accuracy is not None and acuracia_validacao >= target_accuracy:
            print(f"Parada antecipada: Acurácia de validação ({acuracia_validacao:.4f}) atingiu ou excedeu a acurácia alvo da AlexNet ({target_accuracy:.4f}) na época {epoca + 1}")
            break
