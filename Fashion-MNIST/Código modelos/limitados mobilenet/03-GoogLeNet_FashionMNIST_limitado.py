...
nome_modelo = 'GoogleNet-FP32_limitado'
...
class GoogleNet(nn.Module):
    def __init__(self, num_classes=10, dropout_rate=0, width_multiplier=1):
        ...
        self.pre_layers = nn.Sequential(
            nn.Conv2d(1, int(64 * width_multiplier), kernel_size=3, stride=1, padding=1),
...
def treinar_e_validar(modelo, carregador_treino, carregador_validacao, criterio, otimizador, epocas, i, target_accuracy=None):
    ...
    for epoca in range(epocas):
        ...
        if target_accuracy is not None and acuracia_validacao >= target_accuracy:
            print(f"Parada antecipada: Acurácia de validação ({acuracia_validacao:.4f}) atingiu ou excedeu a acurácia alvo da GoogLeNet ({target_accuracy:.4f}) na época {epoca + 1}")
            break
