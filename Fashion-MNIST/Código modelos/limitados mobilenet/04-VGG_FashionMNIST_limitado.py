...
# Constantes para o modelo MobiLeNet
MOBILENET_TARGET_ACCURACY = 0.89
# observado é 0.88
...
# Cria o diretório pai 'VGG16' com incremento, se necessário
nome_modelo = 'VGG16-FP32_limitado'
diretorio_pai = criar_diretorio_incrementado('resultadosVGG16', nome_modelo)
print(f'Diretório criado: {diretorio_pai}')

# Cria o diretório 'AlexNetCarbon'
diretorio_carbon = criar_diretorio_incrementado(diretorio_pai, 'VGG16_carbono')
print(f'Diretório Carbono criado: {diretorio_carbon}')

# Carrega e normaliza o Fashion MNIST
transformacao = transforms.Compose([transforms.ToTensor(),
                                     transforms.Normalize((0.5, ), (0.5, )),
                                     transforms.RandomHorizontalFlip(),
                                    ])

conjunto_treino_completo = datasets.FashionMNIST(root='./data', train=True, download=True, transform=transformacao)
conjunto_teste = datasets.FashionMNIST(root='./data', train=False, download=True, transform=transformacao)
...
class EarlyStopping:
    def __init__(self, patience=5, min_delta=0.001):
        ...
        min_delta (float): Melhoria mínima na perda monitorada para ser considerada uma melhoria.
        """
    ...
    def __call__(self, val_loss):
        if self.best_loss is None:
            ...
            self.counter += 1
            print(f'Paciencia: {self.counter}/{self.patience} - Perda de validação não melhorou por {self.min_delta:.4f}')
...
def treinar_e_validar(modelo, carregador_treino, carregador_validacao, criterio, otimizador, epocas, i, target_accuracy=None):
    ...
    for epoca in range(epocas):
        ...
        if target_accuracy is not None and acuracia_validacao >= target_accuracy:
            print(f"Parada antecipada: Acurácia de validação ({acuracia_validacao:.4f}) atingiu ou excedeu a acurácia alvo da VGG16 ({target_accuracy:.4f}) na época {epoca + 1}")
            break
...
# @track_emissions(project_name="VGG16_original", output_dir=diretorio_carbon)
